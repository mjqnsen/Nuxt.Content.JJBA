    ...
    <ContentDoc>
      <template #default="{ doc }">
        ...
        <div class="mb-8">
          ...
          <div v-if="doc.description" class="text-lg text-muted-foreground mb-6 p-4 bg-muted rounded-lg border-l-4 border-primary">
            {{ doc.description }}
          </div>
        </div>
        ...
        <!-- Additional Images -->
        <div v-if="doc.gallery && doc.gallery.length > 0" class="mt-12">
          <h3 class="text-2xl font-semibold text-foreground mb-6">Project Galerij</h3>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card v-for="(image, index) in doc.gallery" :key="index" class="overflow-hidden">
              <template #header>
                <img 
...
const { data: doc } = await useAsyncData(`nieuws-${route.params.slug}`, () => 
  queryContent(`nieuws/${route.params.slug}`).findOne()
)
...
useHead({
  title: () => doc.value ? `${doc.value.title} - Jan Jansen bouwkundig Adviseurs` : 'Nieuwsartikel - Jan Jansen bouwkundig Adviseurs',
  meta: [
    {
      name: 'description',
      content: () => doc.value?.description || 'Nieuwsartikel van Jan Jansen bouwkundig Adviseurs'
    }
  ]
})
</script>
